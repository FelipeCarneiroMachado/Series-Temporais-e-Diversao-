...
crimes_mensal_sp_wide |> 
  ...
  geom_line() +
  labs(title="Serie de Roubos de Carros") +
  scale_x_date(
    NULL,
...
h <- length(test.roubos_ts)
plot(roubos_ts, main= "Comparação final entre modelos", ylab="Roubos", xlab="Mes")

previsao_hw <- predict(hw.fit, n.ahead=h)
lines(previsao_hw, col="red", lw=1.5)

previsao_sarima <- predict(modelo_sarima, n.ahead=h)
lines(previsao_sarima$pred, col="blue", lw=1.5)

legend("topright", legend = c("Real", "Holt Winters", "SARIMA"), col=c("black", "red", "blue"), lty=1, cex=0.8)
```
